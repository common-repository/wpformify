...
registerBlockType(
	'wpep/shortcode',
	{
		...
		title: 'WP Formify Form',
		description: 'Block to add WP Formify shortcode to the page',
		icon: 'format-aside',
		category: 'layout',
		attributes: {
			type: {
		...
		edit( props ) {
			...
			function wpep_shortcode_change(e) {
				...
				// < img style = {wpep_logo} src = {'https://wpeasypay.com/wp-content/uploads/2019/12/Group-270@2x.png'} / >
			}

			return (
			< div style = {wpep_block_container} > < div style = {divStyle} >  </div> < div > < select onChange = {wpep_shortcode_change} > {options} </select> </div> </div>
		);
