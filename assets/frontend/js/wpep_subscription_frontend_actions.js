		...
		jQuery.post(
			...
			function(response) {
				...
				if (response == 'success') {
					...
					if ( This.val() == 'Pause' ) {
						p.html( "<input type='button' value='Start' data-action='Active' data-subscription='" + subscription_id + "' class='wpep_subscription_action wpep-btn wpep-btn-primary wpep-full wpep-btn-square wpep-btn-success' />" );
						jQuery( `.wpep_sub_status_${subscription_id}` ).html( '<span class="Paused">Paused</span>' );
					} else {
						p.html( "<input type='button' value='Pause' data-action='Paused' data-subscription='" + subscription_id + "' class='wpep_subscription_action wpep-btn wpep-btn-primary wpep-full wpep-btn-square wpep-btn-warning' />" );
...
jQuery( 'div.wpep-subscription-tabs > a' ).on(
	'click',
	function(e){
		e.preventDefault();
